Use the following knowledge to composite client and server components for optimal performance.
...
Good to know: The corresponding client-only package can be used to mark modules that contain client-only logic like code that accesses the window object.